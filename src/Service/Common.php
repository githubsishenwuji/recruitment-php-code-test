 ...
 *
 * geoHelperAddress：
 * 不知道你们两个接口获取的坐标有什么不一样的地方，两个接口的响应时间怎么样，该方法调用的频率是否高
 * 如果很高的话，这个接口的缓存是有问题的，这里只是缓存了Thrift正常返回的结果，如果Thrift返回的接口有问题
 * 压力就全到另外一个接口上，这里建议应该把最终的结果统一缓存
 *
 * checkStatusCallback：
 * 1、状态码建议抽取出来定义
 * 2、代码不够严谨，状态码如果不在定义的范围内就会报错，应该做状态码边界判断
 */
class Common
{ 
